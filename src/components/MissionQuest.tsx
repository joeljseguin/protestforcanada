...
import { ExternalLink, Zap, Phone, Pen, Calendar, MapPin, Users, ChevronDown, ChevronUp, AlertTriangle, CheckCircle2, Circle } from "lucide-react";
    ...
    <div className="py-16 md:py-24">
      <div className="container">
        ...
        <p className="text-muted-foreground font-mono text-sm mb-10 max-w-2xl">
          Select a mission. See the human cost. Take action. Earn XP.
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {missions.slice(0, 5).map((m) => (
            <button
              key={m.id}
              onClick={() => setExpandedMission(m.id)}
        ...
        {mission && (
          <div className="space-y-8 animate-fade-in">
              ...
              <ul className="space-y-3">
                {mission.humanCost.map((item, i) => (
                  <li key={i} className="flex items-start gap-3 font-mono text-sm">
                    <span className="shrink-0 w-6 h-6 neu-border flex items-center justify-center text-xs font-bold bg-secondary">
                      {i + 1}
                    </span>
                    <span>{item}</span>
                  </li>
              ...
              {mission.stats.map((stat) => (
                <div key={stat.label} className="neu-border neu-shadow p-4 bg-card">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-1 mt-3 text-[10px] font-mono text-muted-foreground hover:text-foreground neu-border px-2 py-0.5"
                  >
            ...
            {/* Action Center */}
            <div className="neu-border neu-shadow p-6 bg-card">
              <h4 className="font-heading font-extrabold text-xl uppercase mb-6">Action Center</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {mission.id === "tax" ? (
                  <a
                    href="https://petitions.ourcommons.ca"
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <>
                    <div className="neu-border p-4 bg-card text-center">
                      ...
                      <div className="font-bold text-sm uppercase font-heading">Sign Petition</div>
                      <div className="text-xs text-muted-foreground font-mono mt-1">ourcommons.ca</div>
                      <div className="mt-2 font-mono text-xs font-bold flex items-center justify-center gap-1">
                        <Zap className="h-3 w-3" /> +500 XP
                      </div>
                    </div>
                    <div className="neu-border p-4 bg-card text-center">
                      <Phone className="h-6 w-6 mx-auto mb-2" />
                      <div className="font-bold text-sm uppercase font-heading">Call Your MP</div>
                ...
                {calendarEvents.map((event) => (
                  <div key={event.id} className="neu-border p-4 flex items-center gap-4 bg-card hover:bg-secondary transition-colors">
                    ...
                    <div className="shrink-0 flex items-center gap-3">
                      ...
                      <div className="neu-border px-2 py-1 bg-secondary font-mono text-xs font-bold flex items-center gap-1">
